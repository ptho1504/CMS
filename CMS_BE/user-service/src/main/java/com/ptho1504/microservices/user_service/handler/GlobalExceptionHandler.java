...
import com.ptho1504.microservices.user_service.user.ErrorResponseGrpc;
...
public class GlobalExceptionHandler {
    ...
    public ApiResponse<Object> handle(UserNotFoundException e, HttpServletRequest request) {
    ...
    public ApiResponse<Object> handle(UserExistingException e, HttpServletRequest request) {
        logger.error("{}", e.getMessage());
        return ResponseUtil.error(Arrays.asList(e.getMessage()), e.getMessage(), e.getErrorCode(),
                request.getRequestURI());
    }
    ...
    public StatusRuntimeException handleGrpcUserExisting(UserExistingExceptionGrpc e) {
        ...
        var statusRuntimeException = Status.ALREADY_EXISTS.withDescription("User existing found with email")
